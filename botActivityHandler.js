    ...
    */
    ...
    async handleTeamsMessagingExtensionQuery(context, query) {
        ...
        response.data.objects.forEach(obj => {
            const heroCard = CardFactory.heroCard(obj.package.date); //never display but we required it to add preview invoke
            const preview = CardFactory.heroCard(obj.package.name, `version: ${obj.package.version}`); // list item
            const passData = { description: obj.package.description, test: '111111111111' } //can be any obj
    ...
    async handleTeamsMessagingExtensionSelectItem(context, obj) {
        ...
        return {
            composeExtension: {
                type: 'result',
                attachmentLayout: 'list',
                attachments: [CardFactory.heroCard("Matt Hidinger", obj.test, [{ url: "https://adaptivecards.io/content/cats/3.png" }])]
            }
        };
    }
